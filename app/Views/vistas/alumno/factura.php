        ...
        <div class="card-personalizada">
            ...
            <table class="table table-personalizada mb-4">
                ...
                <tbody>
                    <tr>
                        <td>
                            <span class="fw-bold">Asesoría Académica: <?= esc($pago['materia']) ?></span><br>
                        ...
                        <td class="text-end fw-bold">$<?= number_format($pago['monto'] / 1.16, 2) ?></td>
                    </tr>
                </tbody>
            </table>
            ...
            <div class="row">
                <div class="col-7">
                    <div class="p-3 bg-light rounded">
                        ...
                        <span class="badge-ucot badge-confirmada"><?= strtoupper($pago['estado_pago']) ?></span>
                    </div>
                </div>
                <div class="col-5">
                    <table class="table table-sm table-borderless">
                        <tr><td class="text-end">Subtotal:</td><td class="text-end">$<?= number_format($pago['monto'] / 1.16, 2) ?></td></tr>
                        <tr><td class="text-end">I.V.A. (16%):</td><td class="text-end">$<?= number_format($pago['monto'] - ($pago['monto'] / 1.16), 2) ?></td></tr>
                        <tr class="border-top"><td class="text-end fw-bold">TOTAL USD:</td><td class="text-end fw-bold text-primary">$<?= number_format($pago['monto'], 2) ?></td></tr>
                        <tr>
                            <td class="text-end fw-bold">TOTAL BS:</td>
                            <td class="text-end fw-bold text-dark">Bs. <?= number_format($pago['monto'] * $tasa_bcv, 2, ',', '.') ?></td>
                        </tr>
                    </table>
                    <p class="text-end text-muted small">Tasa BCV: Bs. <?= number_format($tasa_bcv, 2, ',', '.') ?></p>
                </div>
            </div>

            <div class="text-center no-print mt-5">
                <button onclick="window.print()" class="btn btn-info text-white rounded-pill px-4">
                    <i class="fas fa-print"></i> Imprimir Factura
                </button>
                <a href="<?= base_url('alumno/mis_citas') ?>" class="btn btn-secondary rounded-pill px-4">Volver</a>
            </div>
        </div>
    </div>
</div>
